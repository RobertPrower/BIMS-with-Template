...
if($_SERVER['REQUEST_METHOD']=="POST"){
    ...
    $expirationdate =$expirationday.' '. $tagalogexpiremonth .' '. $expirationyear;
        ...
        $fname=sanitizeData(utf8_decode($_POST['first_name']?? null));
        $mname=sanitizeData(utf8_decode($_POST['middle_name']?? null));
        $lname=sanitizeData(utf8_decode($_POST['last_name']?? null));
        ...
        $business_name = sanitizeData($_POST['business_name']?? null);
        $business_hnum= sanitizeData($_POST['business_hnum']?? null);
        $business_street= sanitizeData($_POST['business_street']?? null);
        $business_subd = sanitizeData($_POST['business_subd']?? null);
        $businessaddress = $business_hnum .' '. $business_street. ' '. $business_subd;
        $business_type= sanitizeData($_POST['business_type']?? null);

        $checkifempty = [$fname, $lname, $ID, $isResident, $presentedid, $IDnumber, $business_name, $business_hnum, $business_street, $business_type];
