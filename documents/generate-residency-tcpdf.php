...
if($_SERVER['REQUEST_METHOD']=="POST"){
    ...
    $presentedid=sanitizeData($_POST['presented_id']?? "Driver's License");
    $IDnumber=sanitizeData($_POST['id_num']?? "N42-2010642");
    $purpose = sanitizeData($_POST['purpose']?? "Meralco");

    $checkifempty =[$residentno, $issuingdeptno, $username , $rsince, $completeaddress, $fname, $lname, $presentedid, $IDnumber, $purpose];
    ...
    try{
        ...
        $auditTrailStmt->execute([$issuingdeptno, $username]);
        ...
        $fetchusername="SELECT username FROM tbl_username WHERE username_id = ?";
        $stmt=$pdo->prepare($fetchusername);
        $stmt->execute([$username]);
        $userrealname = $stmt->fetchColumn();
    ...
    class MYPDF extends TCPDF {
        ...
        foreach($brgydetailsraw as $brgydetails){
            ...
            if(isset($logo[0])){
                $this->Image("../img/logos/".$logo[0], 10, 5, 25, '', 'PNG', '', 'T', false, 300, '', false,
                 false, 0, false, false, false);
            }
            if(isset($logo[1])){
                $this->Image("../img/logos/".$logo[1], 35, 7, 23, '', 'PNG', '', 'T', false, 300, '', false, 
                false, 0, false, false, false);
            }
            if(isset($logo[2])){
                $this->Image("../img/logos/".$logo[2], 34, 5, 158, '', 'PNG', '', 'T', false, 300, '', false,
                 false, 0, false, false, false);
            }
            if(isset($logo[3])){
                $this->Image("../img/logos/".$logo[3], 170, 7, 24, '', 'PNG', '', 'T', false, 300, '', false,
                 false, 0, false, false, false);
            }
    ...
    $html =
        ...
        $html .=    '   </td>
                <td class="certbody">
                    ...
                    is bonafide resident of this barangay located at<b class="bold">'.'  '.$completeaddress.' '.'</b> SINCE <b class="bold">'.' '.$rsince.
                    ' UP TO PRESENT</b>.
                    ...
                    <p>Given this <b>'.date("d").'th day of  '.date("F").', '.date('Y').'</b>, at Barangay 177, Cielito Homes Subdivision,
