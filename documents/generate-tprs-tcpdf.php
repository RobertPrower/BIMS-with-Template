...
if($_SERVER['REQUEST_METHOD']=="POST"){
    ...
    $directory = "tprs/";
        ...
        $checkifempty=[$issued_by_no, $issuingdeptno, $ID, $isResident, $fname, $lname, $presentedid, $IDnumber, $toda, $route, $tprstype, $maker, $plateno, $engineno];
        ...
        try{
            ...
            $auditTrailQuery = "INSERT INTO tbl_cert_audit_trail(issued_by_no, issuing_dept_no, datetime_issued)
                                VALUES (?,?, ?)";
            $auditTrailStmt = $pdo->prepare($auditTrailQuery);
            $auditTrailStmt->execute([$issued_by_no, $issuingdeptno, $nowdate]);
    ...
    $pdf->SetXY(22, 266);
    // $pdf->Cell(0, 5, txt: $qrContent, 0, 1, 'L', false, '', 0, false, 'T', 'M');
    ...
    $pdf->Image($imagepath, 70, 190, 20, 20, $imagetype, '', '', false, 300, '', false, false, 0); // X, Y, Width, Height
    ...
    $pdf->Rect(70, 190, 20, 20, 'D');

    $pdf->Rect(155, 190, 20, 20, 'D');

    $pdf->Rect(180, 190, 20, 20, 'D');

    $pdf->Rect(105, 200, 35, 5, 'D');

    $pdf->SetFont('calibri', '', 10);

    $pdf->SetTextColor(0, 0, 0); 

    $pdf->SetXY(162, 208); // Position for the tex
    $pdf->Cell(5, 10, "LEFT", 0, 0, 'C', false, '', 0, false, 'T', 'M');

    $pdf->SetXY(162, 211); // Position for the tex
    $pdf->Cell(5, 10, "THUMBMARK", 0, 0, 'C', false, '', 0, false, 'T', 'M');

    $pdf->SetXY(187, 208); // Position for the tex
    $pdf->Cell(5, 10, "RIGHT", 0, 0, 'C', false, '', 0, false, 'T', 'M');

    $pdf->SetXY(187  , 211); // Position for the tex
    $pdf->Cell(5, 10, "THUMBMARK", 0, 0, 'C', false, '', 0, false, 'T', 'M');

    $pdf->SetXY(120, 208); // Position for the tex
