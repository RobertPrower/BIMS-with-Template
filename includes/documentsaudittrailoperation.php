<?php
    ...
    if($operation_check == "RESIDENT_FETCH_TABLE"){
        ...
        $sqlquery = "SELECT * FROM vw_documents_audit ORDER BY action_timestamp DESC LIMIT :start_from , 10";
        ...
        $sqlquery="CALL FetchDocumentAuditDetails(?)";
        $stmt = $pdo->prepare($sqlquery);
        $stmt->execute([$id]);
        $results = $stmt->fetchAll(PDO::FETCH_ASSOC);
        ...
        $sqlquery="CALL `FetchNewEntryDocumentsAudit`(?)";
        $stmt = $pdo->prepare($sqlquery);
        $stmt->execute([$id]);
        $results = $stmt->fetchAll(PDO::FETCH_ASSOC);
        ...
        if(!empty($search)){
            ...
            $stmt = $pdo->prepare("CALL SearchDocumentsAudit(:search, :start_from)"); 
        ...
        if(!empty($search)){
            ...
            // query to fetch records with pagination
            $stmt = $pdo->prepare("CALL `SearchDocumentsAuditWithDateFilters`(:search, :start_from, :start_date, :end_date)"); 
        ...
        // query to fetch records with pagination
        $stmt = $pdo->prepare("CALL `SearchDocumentsAuditWithDate`(:start_from, :start_date, :end_date)"); 
        ...
        $total_records = $pdo->query("SELECT COUNT(*) FROM vw_documents_audit")->fetchColumn();
