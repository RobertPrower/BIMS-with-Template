<?php
    ...
    if($operation_check == "RESIDENT_FETCH_TABLE"){
        ...
        $sqlquery = "SELECT * FROM vw_nonresident_audit ORDER BY action_timestamp DESC LIMIT :start_from , 10";
        ...
        $sqlquery="SELECT JSON_EXTRACT(action_data, '$.old_values') AS old_values FROM `nonresident_audit` WHERE audit_id=?";
        $stmt = $pdo->prepare($sqlquery);
        $stmt->execute([$id]);
        $results = $stmt->fetchAll(PDO::FETCH_ASSOC);
        ...
        $sqlquery="SELECT JSON_EXTRACT(action_data, '$.new_values') AS old_values FROM `nonresident_audit` WHERE audit_id=?";
        $stmt = $pdo->prepare($sqlquery);
        $stmt->execute([$id]);
        $results = $stmt->fetchAll(PDO::FETCH_ASSOC);
        ...
        $sqlquery="SELECT action_data AS new_entry FROM `nonresident_audit` WHERE audit_id=?";
        $stmt = $pdo->prepare($sqlquery);
        $stmt->execute([$id]);
        $results = $stmt->fetchAll(PDO::FETCH_ASSOC);
        ...
        JSON_EXTRACT(action_data, '$.new_values')
         AS new_entry FROM `nonresident_audit` WHERE audit_id=?";
        $stmt = $pdo->prepare($sqlquery);
        $stmt->execute([$id]);
        $results = $stmt->fetchAll(PDO::FETCH_ASSOC);
        ...
        if(!empty($search)){
            ...
            $stmt = $pdo->prepare("CALL SearchNonResidentAudit(:search, :start_from)"); 
        ...
        if(!empty($search)){
            ...
            // query to fetch records with pagination
            $stmt = $pdo->prepare("CALL SearchNonResidentAuditIWithDate(:search, :start_from, :start_date, :end_date)"); 
        ...
        // query to fetch records with pagination
        $stmt = $pdo->prepare("CALL SearchNonResidentAuditIWithDateFilter(:start_from, :start_date, :end_date)"); 
        ...
        $total_records = $pdo->query("SELECT COUNT(*) FROM vw_nonresident_audit")->fetchColumn();
