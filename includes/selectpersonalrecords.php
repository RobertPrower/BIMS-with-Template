<?php
  foreach ($results as $row) {
    ...
    echo '<td class="text-center">' . htmlspecialchars($row['last_name']) . ', ' . htmlspecialchars($row['first_name']) . ' ' 
    ...
    switch($isResident){
        ...
        break;
        case 0:
            echo '<td class="text-center">' . htmlspecialchars($row['house_num']) . ', ' . htmlspecialchars($row['street']) . $subd .', 
            '.htmlspecialchars($row['district_brgy']).', '.htmlspecialchars($row['city']).', '.htmlspecialchars($row['province']).', '
            .htmlspecialchars($row['zipcode']).'</td>';
        default:
    }
    ...
    switch($isResident){
        case 1:
            echo '<td class="text-center">' . ($row['is_a_voter'] ? '<img width="30" height="30" src="./img/svg/check-solid.png" style="color: #2cfc62"></img>' : 
            '<img width="30" height="30" src="./img/svg/xmark-solid.svg" style="opacity: 40%"></img>') . '</td>';
        break;
        case 0:
        break;
        default:
