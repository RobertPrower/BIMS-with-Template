...
require_once('connecttodb.php');
    ...
    $target_file = $target_dir . basename($_FILES["watermark"]["name"]);
    ...
    $fileName = generateUniqueFileName($target_dir, basename($_FILES["watermark"]["name"]));
    $target_file = $target_dir . $fileName;

    // Check if file is an image
    $check = getimagesize($_FILES["watermark"]["tmp_name"]);
    if ($check === false) {
        throw new Exception("File is not an image.");
    }

    // Check file size
    if ($_FILES["watermark"]["size"] > 5000000) {
    ...
    if (!move_uploaded_file($_FILES["watermark"]["tmp_name"], $target_file)) {
...
$sqlquery="UPDATE `certificate-img` SET filename=? WHERE img_id=4";
